...
import { _store, syncAppStoreToLocalStorage } from "./db-api";
...
export const browserScenarioApi = {
  ...
  save: (name: string): Promise<void> => {
    ...
    rows.push({
      ...
      data: JSON.stringify(buildCurrentPayload())
    });
    writeAll(rows);
    return Promise.resolve();
  },
  ...
  overwrite: (id: string): Promise<void> => {
    ...
    row.data = JSON.stringify(buildCurrentPayload());
  ...
  load: (id: string): Promise<void> => {
    ...
    let payload: Parameters<typeof applyPayloadToStore>[0];
    try {
